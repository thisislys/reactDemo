...
const { login, saveToken } = require("./sql")
            ...
            app.get("/userLogin", (req, res) => {
                ...
                login(option, (error, result) => {
                    if (error) {
                        ...
                        if (result.length > 0) {
                            let token = result[0].userid + new Date().getTime();
                            saveToken(result[0].username, convert2md5(token), (error, result) => {
                                if (!error) console.log("insert success!")
                            })
                            res.json({
                                code: 1,
                                msg: "login success",
                                token: convert2md5(token),
                                datalist: result[0]
                            })
                        } else {
                            res.json({
